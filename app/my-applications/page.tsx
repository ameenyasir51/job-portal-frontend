...
import { ArrowLeft, Building, Calendar, CheckCircle, Clock, XCircle, FileText } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-5">
        <div>
          ...
          <p className="text-gray-400 text-xs font-medium mt-1">Monitor the recruitment pipeline status for your submitted roles.</p>
        </div>
          ...
          {applications.map((app) => (
            <div key={app._id} className="bg-white border border-gray-100 shadow-md rounded-3xl p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:shadow-lg transition-shadow">
              ...
              <div className="space-y-2">
                <h3 className="text-base font-bold text-gray-900">{app.jobId?.title || "Corporate Position"}</h3>
              ...
              <div className="flex sm:flex-col items-end gap-3 w-full sm:w-auto border-t sm:border-t-0 pt-3 sm:pt-0 border-gray-50 justify-between">
                <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 border ${
                  app.status === "shortlisted" ? "bg-green-50 text-green-700 border-green-100" : app.status === "rejected" ? "bg-red-50 text-red-700 border-red-100" : "bg-amber-50 text-amber-700 border-amber-100"
                }`}>
                ...
                {app.resume && (
                  // 🧠 FIXED: Replaced hardcoded localhost reference path string
                  <a href={`${process.env.NEXT_PUBLIC_BACKEND_URL}/uploads/${app.resume}`} target="_blank" rel="noopener noreferrer" className="text-[11px] font-bold text-blue-600 flex items-center gap-1 hover:underline">
